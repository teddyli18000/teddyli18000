...
def raster(name,expected,min_frames=1):
    ...
    if name.endswith('.gif') and frames<min_frames: fail(f'{name}: too few frames ({frames})')
...
def main():
    ...
    for bad in ('live-signal-','field 01','live signal','the repository is the proof surface','motion and system references'):
    ...
    if '<img width="520"' not in readme or '<img width="360"' not in readme: fail('micro motion must stay visibly sized')
    ...
    for n in ('hero-light.gif','hero-dark.gif'): raster(n,(960,300),48)
    for n in ('hero-light.png','hero-dark.png'): raster(n,(960,300))
    for n in ('hero-narrow-light.gif','hero-narrow-dark.gif'): raster(n,(420,180),48)
    for n in ('hero-narrow-light.png','hero-narrow-dark.png'): raster(n,(420,180))
    for n in ('millikan-mark-light.gif','millikan-mark-dark.gif'): raster(n,(520,96),48)
    for n in ('millikan-mark-light.png','millikan-mark-dark.png'): raster(n,(520,96))
    for n in ('sidequest-light.gif','sidequest-dark.gif'): raster(n,(360,92),48)
    for n in ('sidequest-light.png','sidequest-dark.png'): raster(n,(360,92))
    for n in ('live-light.svg','live-dark.svg'):
        ...
        if '<circle' in text or 'LIVE SIGNAL' in text or 'FIELD 01' in text or 'PUBLIC ACTIVITY' in text: fail(f'{n}: decorative status motif returned')
    ...
    if '<circle' in gen: fail('generator can reintroduce status dots')
